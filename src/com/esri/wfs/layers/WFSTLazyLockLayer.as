...
package com.esri.wfs.layers
{
	 ...
	 */
	public class WFSTLazyLockLayer extends GraphicsLayer
	{
		...
		private var ren:WFSTLazyLockRenderer = new WFSTLazyLockRenderer();
		...
		public function cancelTransaction() :void
		{
			...
			if( m_backupGeometry && m_selectedFeature != null )
			{
				...
				m_selectedFeature.symbol = ren.getSymbol( m_selectedFeature );
				m_selectedFeature.refresh();
			}
		...
		private function addFeature( attributes :Object, geometry :Geometry ) :void
		{
			...
			// add the feature to the list of new features
			m_newFeatures.addItem( newFeature );
			add( newFeature );
		...
		private function sendTransaction() :void
		{
			var request :XML =
				<wfs:Transaction xmlns:wfs={WFS} xmlns:ns={ESRI} version={VERSION} service={SERVICE} releaseAction={RELEASE_ACTION}>
					<wfs:LockId xmlns:wfs={WFS}>{m_lockID}</wfs:LockId>
				</wfs:Transaction>;
		...
		private function onMouseOut( event :MouseEvent ) :void
		{
			...
			{
				graphic.symbol =  ren.getSymbol( graphic );
			}
		}
		...
		{
			if( snapEnabledFunction != null && ren.getSymbol != null )
			{
				return snapEnabledFunction();
			}
			return SnapUtil.snappingEnabled( m_snapContext.snapMode ) && ren.getSymbol != null;
		}
