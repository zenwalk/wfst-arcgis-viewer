...
package com.esri.wfs.tasks
{
	...
	import com.esri.wfs.events.WFSQueryEvent;
	...
	{
		//to update: substitute the prvate static const with the WFSComm directly
		private static const GML :Namespace            = WFSComm.GML;
		private static const OWS :Namespace            = WFSComm.OWS;
		private static const WFS :Namespace            = WFSComm.WFS;
		private static const OGC :Namespace            = WFSComm.OGC;
		private static const SERVICE :String           = WFSComm.SERVICE;
		private static const VERSION :String           = WFSComm.VERSION;
		private static const XLINK :Namespace          = WFSComm.XLINK;
		private static const OUTPUT_FORMAT  :String    = WFSComm.OUTPUT_FORMAT;
		...
		{
			trace("creating features...");
			var swapCoordinates :Boolean = query.swapCoordinates;
			var featureSet :FeatureSet = new FeatureSet();
			var wkids :Array = new Array();
			var wkid :Number;

			// retrieve the well known id from the srsName ex: urn:x-ogc:def:crs:EPSG:6.9:4326
			if (xml..GML::Envelope == undefined)
			{
				///////
				//IM NOT GETTING HERE (?)
				//there is something in testing on XML contents that does not work
				//////
				wkids = query.srsName.split( ":" );
			}
			else /*if (xml..GML::Envelope != undefined)*/ 
				wkids = xml..GML::Envelope.@srsName.split( ":" );
			//else
				//Alert.show("not a wellformed WFS response");
			
			//finally
			wkid = new Number( wkids[wkids.length - 1] );
			...
			// Parsing response...
			if (xml..GML::featureMember != undefined)
			{
			...
			// added to handle featureMembers without a featureMember child
			// geoserver's tiger:poi is an excellent example
			else if (xml..GML::featureMembers != undefined) 
			{
				//Alert.show("...using featureMembers");
				//Alert.show(featureMembers.children().toString());
				for each( var featureMembers :XML in xml..GML::featureMembers )
				{
					var chdrn :XMLList = featureMembers.children();

					for each( var chld :XML in chdrn )
					{
 						try {
							var grphc :Graphic = createGraphic( attributes, aliases, chld, query, featureSet.spatialReference );
							//grphc.geometry = WebMercatorUtil.geographicToWebMercator(grphc.geometry);
							features.addItem( grphc );
						} catch (e:Error) {
							trace("ERROR:" + e.message);
						}
					}
				...
				Alert.show("cannot parse wfs response");
			
			//se è 0 allora non ha parsato...
			//Check...
			if (features.length == 0)
			{
				Alert.show("Zero Features loaded!!!");
				return null;
			}
			
			featureSet.features = features.source;
			featureSet.fieldAliases = aliases;
			trace("# of features:" + featureSet.features.length);
			return featureSet;
		}
			...
			spatialReference :SpatialReference ) :void
		{
			...
			for each( var shape :XML in geometry )
			{
				...
				var dimstr :String = geometry.attribute("srsDimension").toString();
				var dim:int;
				if(dimstr != "")
					dim = parseInt(dimstr,10);
				else
					dim = 2;
				...
				{
					graphic.geometry = createPolygon( shape, query.swapCoordinates, spatialReference, dim );
				}
				else if( type == "LineString" || type == "Curve" )
				{
					graphic.geometry = createLine( shape, query.swapCoordinates, spatialReference, dim );
				}
				else if(( type == "MultiSurface" )|| (type == "MultiPolygon"))
				{
					graphic.geometry = createMultiPolygon( shape, query.swapCoordinates, spatialReference, dim );
				}
				else if( type == "MultiCurve" || type == "MultiLineString" )
				{
					graphic.geometry = createMultiLine( shape, query.swapCoordinates, spatialReference, dim ); 
				}
			}
		}
		 ...
		 */
		private function createMultiLine( shape :XML, swapCoordinates :Boolean, spatialReference :SpatialReference, dimension :int ) :Polyline
		{
			...
			for each( var path :String in shape..GML::LineString..GML::posList )
				line.addPath( createPointArray( path, swapCoordinates, spatialReference, dimension ) );
		 ...
		 * @return the Polyline object.
		 */
		private function createLine( shape :XML, swapCoordinates :Boolean, spatialReference :SpatialReference, dimension:int ) :Polyline
		{
			var line :Polyline = null;
			var lines :Object=null;
			try {
				lines = shape..GML::posList;
			}
			catch (e:TypeError)
			{
			...
			if( lines )
			{
				...
				for each( var coordinates :String in lines )
					line.addPath( createPointArray( coordinates, swapCoordinates, spatialReference, dimension ) );
			}
			return line;
		}
		 ...
		 */
		private function createMultiPolygon( shape :XML, swapCoordinates :Boolean, spatialReference :SpatialReference, dimension :int ) :Polygon
		{
			...
			for each( var ring :String in shape..GML::Polygon..GML::posList )
				polygon.addRing( createPointArray( ring, swapCoordinates, spatialReference, dimension ) );

			for each( ring in shape..GML::Surface..GML::posList )
				polygon.addRing( createPointArray( ring, swapCoordinates, spatialReference, dimension ) );

			for each( ring in shape..GML::Polygon..GML::coordinates )
				polygon.addRing( createPointArray( ring, swapCoordinates, spatialReference, dimension ) );
		 ...
		 * @return the Polygon object.
		 */ 
		private function createPolygon( shape :XML, swapCoordinates :Boolean, spatialReference :SpatialReference, dimension :int ) :Polygon
		{
			// grab both the gml:exterior and gml:interior gml:posList elements and let Flash decide what is a hole using the even/odd algorithm
			// if we have to deal with winding direction in the future, simply grab the gml:exterior and gml:interior gml:posList elements separately,
			// and then make sure to reverse the array returned by createRing() for the gml:interior gml:posList
		 ...
		 */ 
		private function createPointArray( coordinates :String, swapCoordinates :Boolean, spatialReference :SpatialReference, dimension :int ) :Array
		{
			...
			if(coordinates.indexOf(",") <= 0)
			{
				...
				for( var i :int = 0; i < coords.length; i = i + dimension )
				{
					if( swapCoordinates )
			...
			{	
				for( var j :int = 0; j < coords.length; j = j + 1 )
				{
					var coord:Array = coords[j].split(",");
					if( swapCoordinates )
						points.push( new MapPoint( coord[j + 1], coord[j], spatialReference ) );
					else
						points.push( new MapPoint( coord[j], coord[j + 1], spatialReference ) );
				}
			}
			return points;
		} 
		...
		private function createPoint( shape :XML, swapCoordinates :Boolean, spatialReference :SpatialReference ) :MapPoint
		{
			...
			if(shape.GML::pos == undefined) {
				coords = shape.GML::coordinates.split( "," );
			} else {
				coords = shape.GML::pos.split( " " );
			}
		...
		{
			var xml :XML = WFSComm.GetFeatureRequestQuery(query.featureNamespace, query.featureName, query.srsName);
			...
			{
				//There's a problem to parse the BBOX, so comment the code below...
				/*				
				//inserted to handle WebMercator
				var xm:Geometry = null;
				var xn:Geometry = null;
				if (query.srsName == "EPSG:4326")
				{
					xm:Geometry = WebMercatorUtil.webMercatorToGeographic((query.extent.xmax);
					xn:Geometry = WebMercatorUtil.webMercatorToGeographic((query.extent.xmin);
					ym:Geometry = WebMercatorUtil.webMercatorToGeographic((query.extent.ymax);
					yn:Geometry = WebMercatorUtil.webMercatorToGeographic((query.extent.ymin);
				}
				else
				{
				...
				*/
				/*
				var filter :XML = 
				<ogc:Filter xmlns:ogc={OGC} xmlns:gml={GML} xmlns:wfs={WFS}>
					<ogc:BBOX>
						<ogc:PropertyName>{query.shapeField}</ogc:PropertyName>
							<gml:Box xmlns:gml={GML} srsName={query.srsName}>
							<gml:coordinates xmlns:gml={GML}>{getCoordinatesComma( query.extent.xmin, query.extent.ymin, query.swapCoordinates )} {getCoordinatesComma( query.extent.xmax, query.extent.ymax, query.swapCoordinates )}</gml:coordinates>
						</gml:Box>
					</ogc:BBOX>
				</ogc:Filter>;
				*/
				
				// with epsg:3857 there's a little problem with the extend... commented.
				/*
				var filter :XML =
					<ogc:Filter xmlns:ogc={OGC} xmlns:gml={GML} xmlns:wfs={WFS}>
						<ogc:BBOX>
							...
							<gml:Envelope xmlns:gml={GML} srsName={query.srsName}>
								<gml:lowerCorner xmlns:gml={GML}>{getCoordinates( query.extent.xmin, query.extent.ymin, query.swapCoordinates )}</gml:lowerCorner>
								<gml:upperCorner xmlns:gml={GML}>{getCoordinates( query.extent.xmax, query.extent.ymax, query.swapCoordinates )}</gml:upperCorner>
							</gml:Envelope>
						</ogc:BBOX>
					</ogc:Filter>;
