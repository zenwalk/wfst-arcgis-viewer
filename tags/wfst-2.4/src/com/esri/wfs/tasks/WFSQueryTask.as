...
package com.esri.wfs.tasks
{
	...
	public class WFSQueryTask extends EventDispatcher
	{
		...
		private static const OUTPUT_FORMAT :String = "GML2";
		...
		private function toFeatures( query :WFSQuery, xml :XML ) :FeatureSet
		{
			...
			var wkid :Number = 4326;
			//Alert.show("set featureSet and swapcoords");
					...
					<ogc:Filter xmlns:ogc={OGC} xmlns:gml={GML}>
						<ogc:BBOX>
							...
							<gml:Box xmlns:gml={GML} srsName={query.srsName}>
								<gml:coordinates>{getCoordinatesComma( query.extent.xmin, query.extent.ymin, query.swapCoordinates )} {getCoordinatesComma( query.extent.xmax, query.extent.ymax, query.swapCoordinates )}</gml:coordinates>
							</gml:Box>
						</ogc:BBOX>
					</ogc:Filter>;
		 ...
		 * @return the coordinate string.
		 */
		private function getCoordinatesComma( x :Number, y :Number, swapCoordinates :Boolean ) :String
		{
			return ( swapCoordinates ? y + "," + x : x + "," + y );
		}
	}
